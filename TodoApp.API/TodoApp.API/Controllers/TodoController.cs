...
namespace TodoApp.API.Controllers
{
    ...
    public class TodoController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("api/deleted-todos")]
        public async Task<IEnumerable<Todo>> GetDeletedTodos()
        {
            var deletedTodos = await _todoDbContext.Todos.Where(t => t.IsDeleted).ToListAsync();
            return deletedTodos;
        }

        [HttpDelete]
        [Route("api/deleted-todos/clear")]
        public async Task<IActionResult> ClearDeletedTodos()
        {
            var deletedTodos = await _todoDbContext.Todos.Where(t => t.IsDeleted).ToListAsync();
            _todoDbContext.Todos.RemoveRange(deletedTodos);
            await _todoDbContext.SaveChangesAsync();
